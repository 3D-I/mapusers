	...
	public function main($id, $mode)
	{
		...
		if ($request->is_set_post('submit'))
		{
			...
			$sql = 'SELECT p.user_id, p.pf_phpbb_location FROM ' . $table_prefix . 'profile_fields_data p ' .
			 			'LEFT JOIN ' . $table_prefix . 'mapusers_geolocation g ' .
        ...
        ' WHERE p.pf_phpbb_location is null';
        $result = $this->db->sql_query($sql);
        ...
        $sql = 'SELECT count(*) as c FROM ' . $table_prefix . 'profile_fields_data p, ' .
            ...
            ' WHERE g.user_id=p.user_id AND p.pf_phpbb_location is not null';
        $result = $this->db->sql_query($sql);
        $row = $this->db->sql_fetchrow($result);
        $loc_geo = $row['c'];
        $this->db->sql_freeresult($result);
