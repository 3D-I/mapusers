...
class main_listener implements EventSubscriberInterface
{
	...
	public function set_permissions($event)
	{
	    ...
	    $permissions['u_mapusers_view'] = array('lang' => 'ACL_U_MAPUSERS_VIEW', 'cat' => 'profile');
	...
	public function add_page_header_link()
	{
	    ...
	    if ($auth->acl_get('u_mapusers_view'))
	    {
		      $this->template->assign_vars(array(
			     'U_MAPUSERS_PAGE'	=> $this->helper->route('myersware_mapusers_controller_main', array('name' => 'showmap')),
		      ));
	    }
	}
	...
	function update_geo_data($user_id, $user_row, $cp_data)
	{
	    ...
	    if ($cp_data['pf_phpbb_location'] == null) {
	        ...
	        $insert = 'INSERT INTO ' . $table_prefix . 'mapusers_geolocation ' .
	   	        '(user_id, latitude, longitude, is_valid, location) VALUES(' .
	   	        $row['user_id'] . ', ' . $geocode['geometry']['location']['lat'] . ', ' .
	   	        $geocode['geometry']['location']['lng'] . ',' .
	   	        '1, "' . $db->sql_escape($address) . '")';
